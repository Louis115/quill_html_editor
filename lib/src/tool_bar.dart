...
class ToolBar extends StatefulWidget {
  ...
  ///toolbarFocusNode
  late final ValueNotifier toolbarFocusNotifier;
  ...
  ToolBar({
    required this.toolbarFocusNotifier,
...
class ToolBarState extends State<ToolBar> {
            ...
            child: ToolBarItem(
              ...
              onTap: () async {
                if (toolbarItem.style == ToolBarStyle.clearHistory) {
                  ...
                  // widget.toolbarFocusNotifier.value = true;
                  // debugPrint(
                  //     "louis check1:${widget.toolbarFocusNotifier.value}");
                  // await ImageSelector(
                  //   onImagePicked: (value) {
                  ...
                  //   },
                  // ).pickFiles();
                  // widget.toolbarFocusNotifier.value = false;
                  widget.toolbarFocusNotifier.value = true;

                  await ImageSelector(
                    onImagePicked: (base64Image) async {
                      // Upload image to Firebase and get URL
                      String imageUrl = await _uploadBase64ToFirebase(base64Image);

                      // Insert image into the editor using URL
                      _formatMap['image'] = imageUrl;
                      widget.controller.embedImage(imageUrl);
                    },
                  ).pickFiles();
        ...
        child: ButtonTheme(
          ...
          child: DropdownButton(
              ...
              items: [
                ...
                _fontSizeItem(type: 'Normal', fontSize: 20),
              ...
              onChanged: (value) {
                debugPrint("Selected font size: $value");
  ...
  Future<String> _uploadBase64ToFirebase(String base64Data) async {
    // Extract Base64 string without metadata
    final RegExp regex = RegExp(r'data:image\/([a-zA-Z]+);base64,');
    final match = regex.firstMatch(base64Data);
    if (match == null) {
      throw FormatException("Invalid Base64 image format!");
    }

    // Extract the file type (e.g., "png", "jpeg")
    final String imageType = match.group(1)!;

    // Remove the data URL prefix
    final String pureBase64 = base64Data.replaceFirst(regex, '');

    // Decode Base64 to bytes
    final Uint8List imageBytes = base64Decode(pureBase64);

    // Upload to Firebase Storage
    final String fileName = 'image_${DateTime.now().millisecondsSinceEpoch}.$imageType';
    final Reference ref = FirebaseStorage.instance.ref().child('uploads/$fileName');
    final UploadTask uploadTask = ref.putData(
      imageBytes,
      SettableMetadata(contentType: 'image/$imageType'),
    );
...
/***
    ...
    [{ 'size': ['small', false, 'large', 'huge','louis'] }],  // custom dropdown
